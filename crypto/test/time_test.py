...
class TestPrice(ut.TestCase):
    ...
    def test_get_yesterday_am_close_price(self):
        ...
        now = dt.datetime.now()
        # 어제 11시
        yesterday_11_am = now.replace(hour=11, minute=0, second=0, microsecond=0) - dt.timedelta(days=1)
        # 어제 12시
        ...
        self.assertEqual(yesterday_am_close_expact.index[0], yesterday_11_am)
    ...
    def test_get_yesterday_am_h1(self):
        ...
        price = utils.Price()
        now = dt.datetime.now()
        yesterday_0_am = now.replace(hour=0, minute=0, second=0, microsecond=0) - dt.timedelta(days=1)
        yesterday_11_am = now.replace(hour=11, minute=0, second=0, microsecond=0) - dt.timedelta(days=1)
        ...
        # then
        self.assertEqual(yesterday_am_h1.index[0], yesterday_0_am)
        self.assertEqual(yesterday_am_h1.index[-1], yesterday_11_am)
...
class TestPriceUtil(ut.TestCase):
    ...
    def test_get_range_ratio(self):
        ...
        # when
        range_ratio = utils.PriceUtils.get_range_ratio(df)
